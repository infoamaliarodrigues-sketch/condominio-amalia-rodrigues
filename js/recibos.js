...
// Mostrar / esconder data de transferência
formaPagamentoSelect.addEventListener("change", () => {
    dataTransferenciaBox.style.display =
        formaPagamentoSelect.value === "transferencia" ? "block" : "none";
});
...
async function gerarRecibo() {
    ...
    const anoFVal = Number(anoFim.value);
    const mesI = Number(mesInicio.value);
    const mesFVal = Number(mesFim.value);
    ...
    const { linhas, total } = await calcularLinhas(fracao, anoI, mesI, anoFVal, mesFVal);
    const extenso = valorPorExtenso(total);

    const periodo = `${String(mesI).padStart(2,"0")}/${anoI} a ${String(mesFVal).padStart(2,"0")}/${anoFVal}`;
...
// Ativar DataTables (versão moderna, sem jQuery)
